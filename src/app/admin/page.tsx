...
export default function AdminPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      const res = await fetch('/api/admin/upload-jobs', { method: 'POST', body: formData })
      const data = await res.json()
      if (res.ok) {
  ...
  return (
    <div className="min-h-screen bg-offwhite">
      ...
      <header className="bg-white border-b border-lightgray">
        ...
        {/* Tab navigation */}
        <div className="flex px-6 gap-0 border-t border-lightgray">
          {tabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`font-sans text-xs px-5 py-3 border-b-2 transition-colors ${
                activeTab === tab.id
                  ? 'border-green-main text-green-deeper font-medium'
                  : 'border-transparent text-midgray hover:text-darkgray hover:border-lightgray'
              }`}
            >
              {tab.label}
            </button>
